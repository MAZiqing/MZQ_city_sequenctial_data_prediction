...
#                     help='dataset_name')
parser.add_argument('--result_path', type=str, default=os.path.join('../result','ANN_result_file.csv'))
...
class Trainer(object):
    def __init__(self, batch_size=args.batch_size):
        ...
        self.model = my_model.ANN(input_size, output_size, output_column, args)
            ...
            if input_p.shape[0] == args.batch_size:
                ...
                if i == 100:
                    if self.epoch % 20 == 0:
                        ...
                        print('output:', np.around(outputs_p[0:10].squeeze().cpu().detach().numpy(), decimals=5))
                loss = self.criterion(outputs_p.squeeze(), label_p[:, -1].squeeze())
                l1loss = self.criterionL1(outputs_p.squeeze(), label_p[:, -1].squeeze())
                loss.backward()
                self.optimizer.step()
