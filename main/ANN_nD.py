...
parser.add_argument('--batch_size', type=int, default=64, metavar='N',
                    help='batch size')
parser.add_argument('--learning_rate', type=float, default=0.000001, metavar='N',
                    help='lr')
...
parser.add_argument('--num_hidden_state', type=int, default=512)
...
#                     help='dataset_name')
parser.add_argument('--result_path', type=str, default=os.path.join('../result', 'ANN_nD.csv'))
...
class Trainer(object):
    def __init__(self, batch_size=args.batch_size):
        ...
        self.model = my_model.ANN_nD(input_size, output_size, output_column, args)
    ...
    def first_write_result(self):
        self.result_df = pd.DataFrame(
            {
                'model_name': [self.model.model_name + '_' + args.mission_name + args.target_sensor],
    ...
    def train(self):
        ...
        for i, (input_p, input_time, label_p, decoder_time, datetime_i) in enumerate(self.data_loader):
            if input_p.shape[0] == args.batch_size:
                outputs_p = self.model(input_p, input_time)  # torch.Size([64, 10, 9])
                label_p = label_p[:, -args.decoder_sequence_length:].squeeze()
                if i == 100:
                    if self.epoch % 20 == 0:
                        print('label:', np.around(label_p[0:10, -1].cpu().detach().numpy(), decimals=5))
                        print('output:', np.around(outputs_p[0:10, -1].squeeze().cpu().detach().numpy(), decimals=5))

                loss = self.criterion(outputs_p.squeeze(), label_p)
                l1loss = self.criterionL1(outputs_p.squeeze(), label_p)
    ...
    def valid(self):
        ...
        for index, (input_p, input_time, label_p, decoder_time, datetime_i) in enumerate(self.data_loader_valid):
            outputs_p = self.model(input_p, input_time)
