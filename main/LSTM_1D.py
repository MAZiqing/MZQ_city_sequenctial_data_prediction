...
#                     help='dataset_name')
parser.add_argument('--result_path', type=str, default=os.path.join('../result', 'LSTM_1D.csv'))
...
class Trainer(object):
    def __init__(self, batch_size=args.batch_size):
        ...
        self.model = my_model.LSTM_1D(input_size, output_size, output_column, args)
    ...
    def train(self):
        ...
        for i, (input_p, input_time, label_p, decoder_time) in enumerate(self.data_loader):
            if input_p.shape[0] == args.batch_size:
                ...
                loss = self.criterion(outputs_p.squeeze(), label_p[:, 1:args.encoder_sequence_length+1])
                l1loss = self.criterionL1(outputs_p.squeeze(), label_p[:, 1:args.encoder_sequence_length+1])
                loss.backward()
                self.optimizer.step()
